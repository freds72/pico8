...
function make_part(x,y,z,src,dx,dy,dz,a)
	local p=clone(part_cls,
		clone(src,{
			...
			angle=a or 0}))
	if(p.sfx) sfx(p.sfx)
	if(p.pause_t) pause_t=p.pause_t
	p.t=time_t+p.dly
	return add(parts,p)
end
